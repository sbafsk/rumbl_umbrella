defmodule RumblWeb.VideoControllerTest do
  ...
  test "authorizes actions against access by other users", %{conn: conn} do
    ...
    assert_error_sent :not_found, fn ->
      put(conn, Routes.video_path(conn, :update, video, video: @created_attrs))
    end
  ...
  describe "with a logged-in user" do
    ...
    test "creates user video and redirects", %{conn: conn, user: user} do
      create_conn = post conn, Routes.video_path(conn, :create), video: @created_attrs

      assert %{id: id} = redirected_params(create_conn)
      assert redirected_to(create_conn) == Routes.video_path(create_conn, :show, id)

      conn = get(conn, Routes.video_path(create_conn, :show, id))
